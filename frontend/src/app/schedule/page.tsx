...
const timeSlots = ["9H", "10H30", "12H", "14H-15H30", "15H30", "17H", "17H-18H30", "18H30-20H"]
  ...
  return (
    <div className="min-h-screen bg-[#1A1E2E] text-white p-4 md:p-8 lg:p-12">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="text-gray-400 mb-6">Horaires des cours et disponibilité des courts</p>
          ...
          {uniqueGroups.map((group) => {
            ...
            return (
              <Button
                key={group}
                variant="outline"
                className={`${selectedFilter === group ? style.color : ""} ${selectedFilter === group ? style.text : ""}`}
        ...
        <Button variant="outline" className="mt-2" onClick={() => setIsLegendVisible(!isLegendVisible)}>
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              {Object.entries(groupTypes).map(([group, styles]) => (
                <div key={group} className={`${styles.color} ${styles.text} px-3 py-1 rounded-md text-sm`}>
                  {group}
                </div>
              ))}
            </div>
      ...
      <div className="w-full overflow-x-auto pb-6">
        <div className="min-w-[1000px]">
          <div className="grid grid-cols-8 gap-1">
            {/* Time column header */}
            <div className="bg-gray-800/50 p-4 font-semibold rounded-tl-lg">Horaire</div>
            ...
            {timeSlots.map((time, timeIndex) => (
              <React.Fragment key={`time-row-${time}`}>
                ...
                {days.map((day) => {
                  ...
                  const scheduleItems = scheduleData.filter((item) => item.day === day && item.time === time)
                      ...
                      {scheduleItems.length > 0 ? (
                        <div className="grid gap-1">
                          {scheduleItems.map((item, index) => {
                            const style = groupTypes[item.group as keyof typeof groupTypes]

                            // Skip if filtered and not matching
                            if (selectedFilter && selectedFilter !== item.group) {
                              return null
                            }
